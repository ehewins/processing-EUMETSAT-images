...
window_size = 7  # number of images averaged over during rolling average
array = VIS6.astype(np.uint32)  # Change VIS6 to VIS8 or IR16 to test them out
...
ax1.imshow(array[7], cmap='gray')
ax2.imshow(rolling_average[7], cmap='gray')
for ax in (ax1, ax2):
    ax.set_axis_off()
